...
public class Camera {
     ...
     */
    public Camera(float aspectRatio, float x, float y, float z, float pitch, float yaw, float roll, float zNear,
                       float zFar) {
        if (aspectRatio <= 0) {
            throw new IllegalArgumentException("aspectRatio " + aspectRatio + " was 0 or was smaller than 0");
        }
    ...
    public void processMouse() {
        ...
        lastDX = mouseDX;
        lastDY = mouseDY;
        Logger.debug("Mouse delta: " + mouseDX + ", " + mouseDY);
        if (yaw + mouseDX >= 360) {
            yaw = yaw + mouseDX - 360;
    ...
    public void moveFromLook(float dx, float dy, float dz) {
        this.z = (float) (dx * (float) cos(toRadians(yaw - 90)) + dz * cos(toRadians(yaw)));
        this.x = (float) (dx * (float) sin(toRadians(yaw - 90)) + dz * sin(toRadians(yaw)));
        this.y = (float) (dy * (float) sin(toRadians(pitch - 90)) + dz * sin(toRadians(pitch)));
        Logger.info("Moving");
    ...
    public void applyTranslations() {
        ...
        glRotatef(lastDY, 1, 0, 0);
        glRotatef(lastDX, 0, 1, 0);
        glTranslatef(-x, -y, -z);
        glPopAttrib();
    }
